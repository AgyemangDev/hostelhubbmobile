...
import { getBooking, ticketDownloadUrl } from "../../../utils/api/unigo";
import { useResumePayment } from "../../../hooks/transport/useResumePayment";

/**
 * The ticket, assembled in HostelHubb from UniGo's booking data.
 *
 * The scannable QR lives on UniGo's official PDF rather than being drawn on
 * this screen. Drawing it here would mean shipping a QR renderer, and saving
 * the PDF would mean shipping expo-file-system and expo-sharing — native
 * modules the installed app does not have, which would take this feature out of
 * OTA reach entirely. Handing the PDF to the phone's browser needs nothing the
 * app isn't already built with, and the passenger ends up holding the same
 * document either way.
 *
 * The reference shown here is the value that QR encodes, so terminal staff can
 * check a booking from this screen alone if the passenger never downloaded.
 */
...
const Ticket = () => {
  ...
  const handleDownload = useCallback(async () => {
    setDownloading(true);
    try {
      const url = ticketDownloadUrl(booking?.groupRef || groupRef);

      // The in-app browser renders the PDF and offers the phone's own save /
      // share controls. If it can't open (some Android devices have no Custom
      // Tabs provider), hand the URL to the system browser instead.
      try {
        await WebBrowser.openBrowserAsync(url);
      } catch {
        const opened = await Linking.canOpenURL(url);
        if (!opened) throw new Error("No browser is available to open your ticket.");
        await Linking.openURL(url);
      }
      setSaved(true);
    } catch (err) {
      Alert.alert("Could not open your ticket", err.message);
    } finally {
      setDownloading(false);
    }
  }, [booking?.groupRef, groupRef]);

  // Straight from payment: push the ticket at them once. UniGo's confirmation
  // emails have never delivered on this deployment, so the passenger has to
  // walk away holding the ticket rather than expecting one to arrive. Later
  // visits from the Bookings tab just show the button — reopening a browser
  // every time the user glances at their trip would be hostile.
  useEffect(() => {
    if (justPaid !== "1" || autoOpened.current) return;
  ...
  const scanRef = String(booking.groupRef || groupRef || "");
  ...
  return (
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.ticket}>
          ...
          {/* Details. An unpaid booking shows only what it costs — seat,
              passenger and reference would all imply a confirmed seat. */}
          ...
          {/* Boarding code — only ever for a seat that is actually confirmed. */}
          {isPaid ? (
            <View style={styles.codeWrap}>
              ...
              <Text style={styles.codeHint}>
                Download your ticket below for the scannable QR code. Terminal staff can also
                check you in with this code.
              </Text>
            </View>
          ) : (
            <View style={styles.codeWrap}>
              ...
              <Text style={styles.codeHint}>Your boarding code appears once payment clears</Text>
            </View>
          )}
        </View>
        ...
        {isPaid && (
          <View style={saved ? styles.savedCard : styles.saveCard}>
            ...
            <Text style={styles.saveText}>
              {saved
                ? "Your ticket opened in the browser — save it to your phone from there. It also stays here in My Trips, so you can open it again any time."
      ...
      >
        <FunctionalButton
          text="Download ticket with QR code"
...
const styles = StyleSheet.create({
  ...
  codeWrap: { alignItems: "center", paddingBottom: 24, paddingHorizontal: 20, gap: 10 },
  codeLabel: {
    fontSize: 10,
    fontWeight: "700",
  ...
  codeHint: { fontSize: 11, color: COLORS.textFaint, textAlign: "center", lineHeight: 16 },
  codePlaceholder: {
    alignSelf: "stretch",
    height: 62,
    borderRadius: 14,
