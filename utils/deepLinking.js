 ...
 * @returns {Promise<string|null>} the hostel id when the link was a hostel link,
 *   so a caller can resume it after sign-in (MainLayout relies on this).
 */
export const handleDeepLinking = async (event) => {
  ...
  try {
    ...
    if (id) {
      ...
      router.push({
        pathname: "/(Details)/[id]",
        // The screen reads `hostelId` (as do all other callers), not `id`.
        params: { hostelId: id },
      });
      return id;
    }
