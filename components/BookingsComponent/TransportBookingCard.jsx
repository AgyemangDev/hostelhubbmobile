...
import { ticketDownloadUrl } from "../../utils/api/unigo";
 ...
 * actual proof of travel — putting the boarding code one glance away, not one
 * navigation away, is the point.
 *
 * The scannable QR is on UniGo's official PDF, which the download button opens
 * in the browser. Rendering the QR here would mean shipping a QR library, and
 * saving the PDF would mean native modules the installed app doesn't have —
 * either one would take this feature out of OTA reach.
 *
 * An UNPAID trip deliberately shows almost none of that. No code, no booking
...
const TransportBookingCard = ({ booking, onPress, onChanged }) => {
  ...
  const handleDownload = async () => {
    setDownloading(true);
    try {
      const url = ticketDownloadUrl(booking.groupRef);
      try {
            ...
            <View style={styles.peek}>
              <View style={styles.codeBox}>
                ...
                <Text style={styles.codeText} selectable>
                  {String(booking.groupRef || booking.partnerRef || "—")}
                </Text>
              </View>
              ...
              >
                {downloading ? (
                  ...
                  <Ionicons name="download-outline" size={15} color={COLORS.button} />
                )}
                <Text style={styles.downloadText}>Download ticket with QR code</Text>
              </TouchableOpacity>
            </View>
          )}
...
const styles = StyleSheet.create({
  ...
  peek: { paddingHorizontal: 16, paddingTop: 14, gap: 14 },
  codeBox: {
    paddingVertical: 12,
  ...
  downloadRow: { flexDirection: "row", alignItems: "center", gap: 7 },
  downloadText: { fontSize: 13, fontWeight: "700", color: COLORS.button },
